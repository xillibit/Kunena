 ...
 * @since 5.0
 */
class KunenaAdminModelUsertypes extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function getItemsFromXML()
	{
	...
	*/
    ...
    $selected = JFactory::getApplication()->getUserState('kunena.usertypes.object');

		$rank_list = array();
		foreach ( $ranks as $rank ) {
			$rank_list[] = JHtml::_ ( 'select.option', $rank->rank_id, $rank->rank_image );
		}
		$list = JHtml::_('select.genericlist', $rank_list, 'rank_image', 'class="inputbox"', 'value', 'text', isset($selected->rank_image) ? $selected->rank_image : '' );
